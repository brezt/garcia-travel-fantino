        ...
        <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-3 gap-4 md:h-[600px]">
          <motion.div initial={false} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="md:col-span-2 md:row-span-2 relative rounded-3xl overflow-hidden group min-h-[250px]">
            ...
            <div className="absolute bottom-0 left-0 p-8">
              <span className="bg-brand-teal text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider mb-3 inline-block">Más popular</span>
              <h3 className="text-3xl font-black text-white font-montserrat uppercase">Punta Cana</h3>
              <p className="text-white/80 text-sm mt-2 max-w-md">El destino ideal para playas de arena blanca y resorts todo incluido de primera categoría.</p>
              <button onClick={() => openWhatsApp("Hola, me interesa obtener información sobre viajes a Punta Cana.")} className="mt-4 bg-white text-brand-navy text-xs font-bold px-6 py-2 rounded-full uppercase tracking-wider hover:bg-brand-teal hover:text-white transition-colors">Descubrir</button>
            </div>
          </motion.div>
          ...
          <motion.div initial={false} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="md:col-span-1 md:row-span-1 relative rounded-3xl overflow-hidden group min-h-[200px]">
            <Image src="https://images.unsplash.com/photo-1622396481328-9b1b78cdd9fd?auto=format&fit=crop&w=1200&q=80" alt="Samaná" fill sizes="(min-width: 768px) 33vw, 100vw" className="object-cover transition-transform duration-700 group-hover:scale-105" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
            <div className="absolute bottom-0 left-0 p-6">
              <h3 className="text-xl font-black text-white font-montserrat uppercase">Samaná</h3>
          ...
          <motion.div initial={false} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="md:col-span-2 md:row-span-1 relative rounded-3xl overflow-hidden group min-h-[200px]">
            ...
            <div className="absolute bottom-0 left-0 p-6 w-full flex justify-between items-end">
              <div>
                ...
                <p className="text-white/80 text-sm mt-1">Cultura, teleférico y encanto colonial.</p>
              </div>
              <button onClick={() => openWhatsApp("Hola, me interesa obtener información sobre viajes a Puerto Plata.")} className="text-brand-teal hover:text-white transition-colors"><ArrowRight size={24} /></button>
            </div>
          </motion.div>
          ...
          <motion.div initial={false} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="md:col-span-1 md:row-span-1 bg-brand-teal rounded-3xl p-8 flex flex-col justify-center relative overflow-hidden group min-h-[200px]">
            ...
            <button onClick={() => openWhatsApp("Hola, me interesan las opciones de viajes internacionales y cruceros.")} className="bg-white text-brand-teal text-[10px] font-bold px-4 py-2 rounded-full uppercase tracking-wider self-start relative z-10 hover:bg-brand-navy hover:text-white transition-colors">Conocer más</button>
          </motion.div>
        </div>
      </div>
    </section>
